...
function roleBaseDamage(role) {
  const m = { burst: 244, dps_sustain: 178, dps_cycle: 168, board_control: 144, control: 136, survival: 110 };
  return m[role] || 151;
}
...
function scoreHero(hero, terrainTags, bossTags, state) {
  ...
  s -= Number(state.heroUsage[hero.heroId] || 0) * 2.3;
  if (state.lastFailedTeam.has(hero.heroId)) s -= 3.5;
  return s;
}

function chooseTeam(heroes, terrainTags, bossTags, state) {
  const ranked = heroes
  ...
  for (const it of ranked) {
    ...
    if (out.some((x) => x.combatArchetype === it.hero.combatArchetype) && out.length < 3) continue;
    out.push(it.hero);
  }
  for (const it of ranked) {
    if (out.length >= 4) break;
...
function simulateWave(runtime, waveObj, team, state) {
  ...
  const lateWaveAdapt = waveIndex >= 12 ? 1 + (waveIndex - 12) * 0.02 : 1.0;
  const losingChainAdapt = state.loseChain > 0 ? 1 + Math.min(0.08, state.loseChain * 0.03) : 1.0;

  const heroDmgScale = econGrowth * streakGrowth * catchup * replan * bonus.heroMul * lateWaveAdapt * losingChainAdapt;
  const heroHpScale = (1 + (waveIndex - 1) * 0.03) * (1 + state.winStreak * 0.02) * (state.lastLose ? 1.08 : 1) * (waveIndex >= 12 ? 1.05 : 1);
  const bossScale = 1 + (waveIndex - 1) * 0.0165;

  let bossHp = Math.max(1, Math.floor(Number(boss.baseHp || 5000) * bossScale));
  let teamHp = team.map(() => Math.floor(2420 * heroHpScale));
  const turns = [];
...
function main() {
  ...
  const replay = {
    meta: {
      version: "1.0-sim-v12-latewave-adapt",
  ...
  for (const w of waves) {
    ...
    const team = chooseTeam(heroes, terrainTags, bossTags, state);
  ...
  fs.writeFileSync("battle_replay_v12_latewave_adapt.json", JSON.stringify(replay, null, 2), "utf8");
  console.log("battle_replay_v12_latewave_adapt.json generated");
  console.log("SIM_V12_SUMMARY", replay.summary);
}
