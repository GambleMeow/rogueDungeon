...
const OUT_ROOT = `${ROOT}/outputs/batch/elite`;
...
const ELITE_MODELS = [
  ...
  "ArthaswithSword.mdx",
  "herowarden.mdx",
  "az_doomdragon_v1_big.mdx",
  "AZ_Dragonus_C.MDX",
  "Fireravengodmount.mdx",
  "celestialserpent.mdx",
];
...
async function main() {
  ...
  for (const elite of ELITE_MODELS) {
    const modelName = path.basename(elite, ".mdx");
    const modelDir = `${OUT_ROOT}/${modelName}`;
    ensureDir(modelDir);

    const modelPathInMpq =
      fileMap.get(elite.toLowerCase()) || byBaseName.get(path.basename(elite).toLowerCase());
    if (!modelPathInMpq) {
      results.push({ model: elite, ok: false, reason: "地图中未找到该MDX" });
      continue;
    }
    ...
    try {
      ...
      for (const tex of texImages) {
        ...
        if (!texPathInMpq) texPathInMpq = byBaseName.get(path.basename(tex).toLowerCase());
        if (texPathInMpq) extractFromMpq(texPathInMpq, modelDir);
      }

      const outGlb = `${modelDir}/${modelName}.glb`;
      run("node", [CONVERTER, "--input", localMdxPath, "--output", outGlb], `转换${modelName}`);
      ...
      results.push({ model: elite, ok: false, reason: String(err.message || err) });
    }
  }
