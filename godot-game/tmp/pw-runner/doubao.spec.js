...
test("豆包图生图 - PASBTNBash", async ({ page }) => {
  ...
  try {
    ...
    if (loginVisible) {
      ...
      report.steps.push("检测到登录提示，停止后续自动操作");
      return;
    }

    const uploadUsed = await uploadFile(page, SOURCE_IMAGE);
    if (!uploadUsed) {
      throw new Error("未找到可用上传入口");
    }
    report.steps.push(`上传参考图成功，路径: ${uploadUsed}`);
    ...
    report.steps.push(`填入提示词成功，输入框: ${promptUsed}`);

    const ratioUsed = await clickFirstVisible(page, [
      "button:has-text('1:1')",
      "[role='button']:has-text('1:1')",
    ]);
    if (ratioUsed) {
      report.steps.push("尝试设置 1:1 比例");
    }

    const qualityUsed = await clickFirstVisible(page, [
      "button:has-text('高清')",
      "button:has-text('高质量')",
    ...
    report.steps.push(`点击生成按钮: ${generateUsed}`);

    await page.waitForTimeout(45000);
    const afterCount = await page.locator("img").count();
    report.steps.push(`等待完成，img 数: ${beforeCount} -> ${afterCount}`);
