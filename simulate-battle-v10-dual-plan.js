...
function roleBaseDamage(role) {
  const m = { burst: 240, dps_sustain: 175, dps_cycle: 165, board_control: 142, control: 134, survival: 108 };
  return m[role] || 150;
}
...
function scoreHero(hero, terrainTags, bossTags, state) {
  ...
  s -= Number(state.heroUsage[hero.heroId] || 0) * 2.5;
...
function makeTeamFromRanked(ranked, mode) {
  const out = [];
  for (const it of ranked) {
    if (out.length >= 4) break;
    if (mode === "anti_repeat" && out.some((x) => x.combatArchetype === it.hero.combatArchetype)) continue;
    out.push(it.hero);
  }
  for (const it of ranked) {
    if (out.length >= 4) break;
...
function bossSkillDamage(skillId, terrainTags, coef, bossScale) {
  ...
  if (id.includes("charge") || id.includes("burst")) d = 156;
  if (id.includes("ring") || id.includes("nova")) d = 136;
...
function teamBonus(team, terrainTags, bossTags) {
  ...
  return { heroMul: mul, takenMul: defense >= 2 ? 0.93 : 1.0 };
}

function quickEstimate(team, terrainTags, bossTags, state, waveIndex) {
  const b = teamBonus(team, terrainTags, bossTags);
  const econ = 1 + (waveIndex - 1) * 0.04;
  const streak = 1 + state.winStreak * 0.03;
  const catchup = state.lastLose ? 1.1 : 1.0;
  let offense = 0;
  for (const h of team) {
    const a = pickAction(h);
    offense += roleBaseDamage(String(a?.role || "dps_cycle"));
  }
  return offense * econ * streak * catchup * b.heroMul * b.takenMul;
}
...
function simulateWave(runtime, waveObj, team, state) {
  ...
  const catchup = state.lastLose ? 1.1 : 1.0;
  const replan = state.replanCount > 0 ? 1 + Math.min(0.06, state.replanCount * 0.02) : 1.0;

  const heroDmgScale = econGrowth * streakGrowth * catchup * replan * bonus.heroMul;
  const heroHpScale = (1 + (waveIndex - 1) * 0.03) * (1 + state.winStreak * 0.02) * (state.lastLose ? 1.07 : 1);
  const bossScale = 1 + (waveIndex - 1) * 0.017;

  let bossHp = Math.max(1, Math.floor(Number(boss.baseHp || 5000) * bossScale));
  let teamHp = team.map(() => Math.floor(2360 * heroHpScale));
  const turns = [];
...
function main() {
  ...
  const replay = {
    meta: {
      version: "1.0-sim-v10-dual-plan",
      generatedAt: "2026-03-10",
      mapId: 180750,
      adaptiveTeamPool: true,
      dualPlanEstimator: true
    },
    waveReplays: []
  };
  ...
  for (const w of waves) {
    ...
    const teamA = makeTeamFromRanked(ranked, "normal");
    const teamB = makeTeamFromRanked(ranked, "anti_repeat");
    const eA = quickEstimate(teamA, terrainTags, bossTags, state, Number(w.wave));
    const eB = quickEstimate(teamB, terrainTags, bossTags, state, Number(w.wave));
    const team = eB > eA ? teamB : teamA;
  ...
  fs.writeFileSync("battle_replay_v10_dual_plan.json", JSON.stringify(replay, null, 2), "utf8");
  console.log("battle_replay_v10_dual_plan.json generated");
  console.log("SIM_V10_SUMMARY", replay.summary);
}
