...
function terrainCoef(terrainId) {
  const map = {
    ...
    arena_stack_spread_switch: { heroDmgMul: 0.97, bossDmgMul: 1.05 }
  };
...
function scoreHeroForWave(hero, terrainTags, bossTags) {
  ...
  const arch = String(hero.combatArchetype || "hybrid");
  let s = 0;
  ...
  if (bossTags.includes("zone_pressure") && f.defense) s += 7;
  if (bossTags.includes("reposition_pressure") && f.move) s += 7;
  if (bossTags.includes("summon_pressure") && f.control) s += 6;
  if (arch === "hybrid") s += 3;
  if (arch === "summoner" && terrainTags.includes("spread_requirement")) s -= 4;
...
function chooseTeam(allHeroes, terrainTags, bossTags) {
  ...
  for (const it of scored) {
    ...
    const ar = String(it.hero.combatArchetype || "hybrid");
    const limit = ar === "hybrid" ? 2 : 2;
    if ((archCount[ar] || 0) >= limit) continue;
    out.push(it.hero);
    archCount[ar] = (archCount[ar] || 0) + 1;
  }
  for (const it of scored) {
    if (out.length >= 4) break;
...
function bossSkillDamage(skillId, tags, coef) {
  ...
  if (tags.includes("fan_facing")) d = Math.floor(d * 1.06);
...
function simulateWave(runtime, waveObj, allHeroes) {
  ...
  for (let turn = 1; turn <= 12; turn++) {
    ...
    for (let i = 0; i < team.length; i++) {
      ...
      const dmg = Math.floor(roleBaseDamage(role) * offenseMul * coef.heroDmgMul);
      heroTurnDamage += dmg;
...
function main() {
  ...
  const replay = {
    meta: { version: "1.0-sim-v5", generatedAt: "2026-03-10", mapId: 180750, dynamicTeamSelection: true },
    waveReplays: []
  };
  ...
  fs.writeFileSync("battle_replay_v5.json", JSON.stringify(replay, null, 2), "utf8");
  console.log("battle_replay_v5.json generated");
  console.log("SIM_V5_SUMMARY", replay.summary);
}
